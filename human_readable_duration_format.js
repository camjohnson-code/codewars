...
const formatDuration = (seconds) => {
  ...
  for (let interval in intervals) {
    if (seconds >= intervals[interval]) {
      const intervalCount = Math.floor(seconds / intervals[interval]);
      result.push(intervalCount + ' ' + (intervalCount === 1 ? interval : interval + 's'));
      seconds -= intervalCount * intervals[interval];
    }
  }

  const lastInterval = result.pop();
  return result.length > 0
    ? result.join(', ') + ' and ' + lastInterval
    : lastInterval;
};
