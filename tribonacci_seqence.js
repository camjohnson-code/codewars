...
// s starting with [0, 1] instead of [1, 1] basically shifts the common Fibonacci sequence by once place,
...
const tribonacci = (signature, n) => {
  ...
  for (let i = 0; i < n; i++) {
    if (i < 3) output[i] = signature[i];
    else output[i] = output[i - 1] + output[i - 2] + output[i - 3];
  }
